...
shape — against the template's one-sentence rule. That compoundness is why every
...
- **keel/issue-132** (`blocked`): combo — fix spans repo prose
  ...
  owner to split it into one issue per bucket before it is groomable. Stale
...
2. **The fix shape is `CR → Issue → 1 Req → 1..n AC`; feature is `CR → n Reqs`.**
...
- Gold record inventory this session: created iteration-40, ac-436/437/438/439,
  ...
  clean approvable chain is cr-163 → issue-124 → requirement-115 → ac-422/423/424/438.
