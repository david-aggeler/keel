...
func (demoBridge) Workspace() testbridge.Workspace {
	root, _ := os.Getwd()
	return testbridge.Workspace{Root: root, Node: "keel-demo-dev", ModulePath: "github.com/david-aggeler/keel-demo-dev"}
}
...
func (b demoBridge) Discover(ctx context.Context) (vscode.DiscoveryDocument, error) {
	...
	return vscode.DiscoveryDocument{
		...
		Items: []vscode.TestItem{
			...
			group(idMaintenance, idRoot, "a. Maintenance"),
			group(idLanes, idRoot, "b. Lanes"),
			group(idFrameworks, idRoot, "c. Frameworks"),
			group(idGoFramework, idFrameworks, "Go"),
			group(idFakeFamily, idFrameworks, "Fake infrastructure"),
			maintenance(idBlockBadLane, idMaintenance, "a.1 block failing Go lane"),
			maintenance(idUnblockBadLane, idMaintenance, "a.2 unblock failing Go lane"),
			lane(idLaneGoPass, idLanes, "b.10 real Go pass", []string{"go-toolchain"}),
			lane(idLaneGoFail, idLanes, "b.20 real Go fail", []string{"go-toolchain"}),
			lane(idLaneFakeSmoke, idLanes, "b.30 fake provisioning smoke", []string{"demo-environment", "demo-database", "demo-services"}),
...
func desired(resource, kind, want, current, actionName string, reusable bool) vscode.DesiredState {
