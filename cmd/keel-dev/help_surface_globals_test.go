...
func TestKeelDevHelpJSONCarriesGlobalFlagsAndRoot(t *testing.T) {
	...
	root := inventory[idx]
	if root.Invocable == nil || *root.Invocable != false {
		t.Fatalf("root invocable = %v, want false", root.Invocable)
	}
...
func TestKeelDevHelpAdvertisesEveryAcceptedGlobalSpelling(t *testing.T) {
...
func captureHelpSurface(t *testing.T, arg string) string {
	...
	stdout, stderr := captureProcessStreams(t, func() {
		if code := run([]string{arg}); code != 0 {
			t.Fatalf("run(%s) exit = %d, want 0", arg, code)
		}
	})
	if stderr != "" {
		t.Fatalf("run(%s) wrote to stderr: %q", arg, stderr)
	}
	return stdout
}
