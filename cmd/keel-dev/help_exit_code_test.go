...
// TestHelpExitCode covers every branch of helpExitCode: no error, a usage
// error carrying the tree's exit code, and any other error. The unresolvable
// help topic path returns a cli.UsageError, so this is the mapping that makes
// that path exit 2 instead of 0.
//
...
func TestHelpExitCode(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			if got := helpExitCode(tc.err); got != tc.want {
				t.Fatalf("helpExitCode(%v) = %d, want %d", tc.err, got, tc.want)
			}
		})
	}
}
