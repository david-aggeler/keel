...
// (file, function) pair and nothing else: keel-dev's newPayloadStream is
// admitted in cmd/keel-dev/main.go, and the same function name in another
// binary's main.go is still a violation. Keying the allowlist on the base name alone would let
...
func TestStreamLintAllowlistIsScopedToItsFile(t *testing.T) {
	...
	for _, binary := range []string{"keel-dev", "keel-future"} {
		...
		writeFile(t, pkg, "main.go", "package main\n\nimport \"os\"\n\nfunc newPayloadStream() { _ = os.Stderr }\n")
	}
	writeFile(t, dir, "go.mod", "module "+modulePath+"\n\ngo 1.25\n")

	err := runLint(dir, lintFixtureFiles(t, dir))
	if err == nil {
		t.Fatal("lint admitted newPayloadStream in cmd/keel-future/main.go on keel-dev's allowlist entry")
	}
	future := filepath.Join("cmd", "keel-future", "main.go")
	if !strings.Contains(err.Error(), "no-raw-stdout-stream: "+future+":5 references os.Stderr in newPayloadStream") {
		t.Fatalf("lint report lacks the keel-future violation:\n%v", err)
	}
	if strings.Contains(err.Error(), filepath.Join("cmd", "keel-dev", "main.go")) {
		t.Fatalf("lint reported keel-dev's allowlisted newPayloadStream:\n%v", err)
	}
}
