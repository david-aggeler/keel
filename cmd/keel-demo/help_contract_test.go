...
// keel-demo. The last one names a command with a handler, which writes a
// payload line to stdout and a .logs record when it runs.
...
func TestKeelDemoRequestedHelpIsServedByKeelCLIIdenticallyInEveryMode(t *testing.T) {
	for _, request := range helpContractRequests {
		t.Run(strings.Join(request, " "), func(t *testing.T) {
			...
			for _, mode := range helpContractModes {
				...
				if strings.Contains(stdout, "workflow inspect run_id=") {
					t.Fatalf("--mode %s %q ran the workflow inspect handler:\n%s", mode, request, stdout)
				}
			}
		})
	}
...
func TestKeelDemoUsageErrorHelpIsWrittenByKeelCLIToStderr(t *testing.T) {
...
// runHelpContract runs keel-demo in an empty working directory with stdout and
