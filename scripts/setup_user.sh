...
#   Go gate baseline — gopls, golangci-lint, govulncheck, gofumpt, shfmt
#   Secret + spell    — gitleaks, codespell
# All Go tools land in $GOBIN (or $GOPATH/bin); codespell via pipx→pip.
...
# ---------------------------------------------------------------------------
# codespell — spell-check on identifiers and prose. Not a Go tool; install via
# pipx (isolated), falling back to `pip install --user`.
# ---------------------------------------------------------------------------
CODESPELL_VERSION="2.3.0"
CODESPELL_BIN="$(command -v codespell 2>/dev/null || true)"
if [[ -n "$CODESPELL_BIN" ]]; then
  echo "codespell already installed: $(codespell --version 2>&1 | head -n1) (${CODESPELL_BIN})"
elif command -v pipx >/dev/null 2>&1; then
  echo "Installing codespell ${CODESPELL_VERSION} via pipx..."
  pipx install "codespell==${CODESPELL_VERSION}"
elif command -v pip3 >/dev/null 2>&1; then
  echo "Installing codespell ${CODESPELL_VERSION} via pip install --user..."
  pip3 install --user "codespell==${CODESPELL_VERSION}"
else
  echo "WARN: neither pipx nor pip3 found — skipping codespell. Install one, then re-run." >&2
fi
...
report codespell "$(command -v codespell 2>/dev/null || true)"
