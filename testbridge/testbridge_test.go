...
func TestCommandSpecOwnsCanonicalBridgeWire(t *testing.T) {
	...
	if len(desired.DesiredState) != 1 || desired.DesiredState[0].Action != "reconcile_during_run" || !desired.DesiredState[0].Owned {
		t.Fatalf("desired state = %+v, want owned reconcile_during_run row", desired.DesiredState)
	}
...
func TestValidationRejectsInvalidProtocolDocuments(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "setup status", doc: vscode.SetupPlan{Version: 1, Devtool: vscode.DevtoolMetadata{Name: "d", Version: "v"}, Workspace: "w", GeneratedAt: now, DesiredState: []vscode.DesiredState{{Resource: "db", Kind: "service", Desired: "up", Current: "down", Status: "bogus", Action: "reuse"}}, Teardown: vscode.SetupPlanTeardown{Policy: "none"}}, want: "invalid status"},
...
func TestValidationCoversClosedEnumsAndRequiredFields(t *testing.T) {
	...
	plan := validPlan()
	plan.DesiredState[0].Resource = ""
	assertInvalid("desired required", plan, "missing required fields")
	plan = validPlan()
	plan.DesiredState[0].Kind = "workspace"
	assertInvalid("desired kind", plan, "invalid kind")
	plan = validPlan()
	plan.DesiredState[0].Action = "setup_required"
