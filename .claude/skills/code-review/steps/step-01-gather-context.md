...
story_key: '' # set at runtime when resolved from in_review story records
---
...
1. **Find the review target.** The conversation context before this skill was triggered IS your starting point — not a blank slate. Check in this order — stop as soon as the review target is identified:
   ...
   **Tier 3 — Story record query.**
   Call `list_story` with `filter={"status":"in_review"}` scoped to the active product. Scan results for `in_review` stories:
   - **Exactly one `in_review` story:** Set `{story_key}` to the story's key. Suggest it: "I found story <story-id> in `in_review` status. Would you like to review its changes? [Y] Yes / [N] No, let me choose". If confirmed, use the story context to determine the diff source (branch name derived from story slug, or uncommitted changes). If declined, clear `{story_key}` and fall through.
   - **Multiple `in_review` stories:** Present them as numbered options alongside a manual choice option. Wait for user selection. If a story is selected, set `{story_key}` and use its context to determine the diff source. If manual choice is selected, clear `{story_key}` and fall through.
   - **None:** Fall through.
