...
nextStepFile: './step-04-density-validation.md'
altStepFile: './step-03-parity-check.md'
...
Detect if PRD follows the standard format and route appropriately — classify as Standard / Variant / Non-Standard, with optional parity check for non-standard formats.
...
Check if the PRD contains the following core sections:
...
- Follows the standard PRD structure closely

**Variant:**

- 3-4 core sections present
- Generally follows standard patterns but may have structural differences
- Missing some sections but recognizable as a structured PRD

**Non-Standard:**

- Fewer than 3 core sections present
- Does not follow the standard PRD structure
...
**Format Classification:** [Standard / Variant / Non-Standard]
**Core Sections Present:** [count]/6
```

### 5. Route Based on Format Classification

**IF format is Standard or Variant:**

Display: "**Format Detected:** {classification}

Proceeding to systematic validation checks..."

Without delay, read fully and follow: {nextStepFile} (step-04-density-validation.md)

**IF format is Non-Standard (< 3 core sections):**

Display: "**Format Detected:** Non-Standard PRD

This PRD does not follow the standard structure (only {count}/6 core sections present).
...
**[A] Parity Check** - Analyze gaps and estimate effort to reach standard PRD parity
...
- IF A (Parity Check): Read fully and follow: {altStepFile} (step-03-parity-check.md)
...
- Core sections checked systematically
- Format classified correctly based on section count
- Findings reported to validation report
- Standard/Variant PRDs proceed directly to next validation step
