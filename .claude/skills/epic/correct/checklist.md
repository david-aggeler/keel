...
<check-item id="1.1">
<prompt>Identify the triggering unit that revealed this issue</prompt>
<action>Document unit ID and brief description</action>
...
<check-item id="2.1">
<prompt>Evaluate current epic containing the trigger unit</prompt>
...
<action>Can the issue be addressed by modifying existing units?</action>
<action>Can new units be added within the current epic structure?</action>
...
<action>Would reverting recently completed units simplify addressing this issue?</action>
<action>Which units would need to be rolled back?</action>
...
<check-item id="6.4">
<prompt>Update epic and unit records to reflect approved changes</prompt>
<action>If epics were added: Call `create_epic` for each new epic (status `planned`)</action>
<action>If epics were removed or deferred: Call `update_epic` with `status=done` or note deferred per policy</action>
<action>If epic scope changed: Call `update_epic` with updated `summary`/`details`/`plan` fields</action>
<action>If units were added: Call `create_change_request` for each new unit (thin husk: title + summary + parent + status=draft); detail at pickup via `/change-request create`</action>
<action>If units were removed or deferred: Call `update_change_request` with appropriate status per team policy</action>
<action>If units were modified: Call `update_change_request` with the updated content</action>
<status>[ ] Done / [ ] N/A / [ ] Action-needed</status>
</check-item>
