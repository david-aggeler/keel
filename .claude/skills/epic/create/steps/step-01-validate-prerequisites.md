...
To validate that all required input documents exist and extract all requirements (FRs, NFRs, and additional requirements from UX/Architecture) needed for epic and unit creation.
...
- FORBIDDEN to start creating epics or units in this step
- Extract requirements from ALL available documents
- POPULATE the working context with extracted requirements

## EXECUTION PROTOCOLS

- Extract requirements systematically from all documents
- Store extracted requirements in working context for Step 2 (no local file written)
...
Welcome {user_name} to comprehensive epic and unit creation!
...
1. `./*prd*.md` (whole document)
2. `./*prd*/index.md` (sharded version)

**Architecture Document Search Priority:**

1. `./*architecture*.md` (whole document)
2. `./*architecture*/index.md` (sharded version)

**UX Design Document Search (Optional):**

1. `./*ux*.md` (whole document)
2. `./*ux*/index.md` (sharded version)

Before proceeding, ask the user if there are any other documents to include for analysis, and if anything found should be excluded. Wait for user confirmation. Once confirmed, store the list of input documents in working context.
...
Review the Architecture document for technical requirements that impact epic and unit creation:

**Look for:**

- **Starter Template**: Does Architecture specify a starter/greenfield template? If YES, document this for Epic 1 Unit 1
...
**IMPORTANT**: If a starter template is mentioned in Architecture, note it prominently. This will impact Epic 1 Unit 1.
...
**CRITICAL**: Do NOT reduce UX requirements to vague summaries. Each UX-DR must be specific enough to generate a unit with testable acceptance criteria. If the UX spec identifies 6 reusable components, list all 6 — not "create reusable components."
...
- Verify each UX-DR is specific enough for unit creation
...
After extraction and confirmation, store in working context:

- Complete FR list
- Complete NFR list
- All additional requirements
- UX Design requirements (if UX document exists)

This working context is carried forward into Steps 2–4; no local artifact file is written at this step.
...
- IF C: Confirm working context is populated, then read fully and follow: ./step-02-design-epics.md
- IF Any other comments or queries: help user respond then [Redisplay Menu Options](#9-present-menu-options)

## CRITICAL STEP COMPLETION NOTE

ONLY WHEN C is selected and all requirements are in working context will you then read fully and follow: ./step-02-design-epics.md to begin epic design step.
...
- Not confirming requirements with user
