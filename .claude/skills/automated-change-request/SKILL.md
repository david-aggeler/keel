...
x-openbrain-source: automated-change-request/v18
x-openbrain-content-source-hash: sha256:fb89a97607253ea3907daa0c04d7c84050e8a07b1f1838978b7a90cfea74b56b
x-openbrain-content-hash: sha256:05daa96d433272e041ca568e219c9d5afb4b5da1bb76c31e65fca77bcb752e5f
---
...
Pickup signal for `dev`: a change_request at `executor=agent` and `status` either
`approved` (not yet claimed) or `in_progress` (already claimed). **The status says the
unit is claimed; the branch says how much work exists** — `in_progress` spans zero work
through complete-and-ready-for-review, so it is evidence of nothing beyond the claim, and
`dev` reads the branch for what is already done. A blocking `formal_review`
(`outcome: follow_up_required`) naming the unit in `subject_refs` carries that round's
**findings**, whoever wrote it — reviewer, post-merge revert, or a `verify` no-op reopen;
its absence means there are no recorded findings, never that the unit is unpickable. A
verb whose precondition does not match **halts** rather than guessing.
