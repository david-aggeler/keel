...
description: 'Implement an approved unit via a linear vertical-slice TDD loop, runnable by a non-resident executor (codex). Use when executor=agent and status is approved (a fresh claim) or in_progress (already claimed, at any amount of work from none to nearly complete), and the operator says "dev this CR headless" or "codex dev".'
x-openbrain-content-hash: sha256:cf37a35dd17af9a1cd757eb136235219f303516743e41932f3849ed783a62840
---

# Automated Dev

**Transition:** `approved → in_progress` (fresh entry) **or** `in_progress → in_progress`
(already claimed) → `implementation_review` (on a complete, green, non-empty-diff unit)
...
3. Confirm the status is one of the **two** admissible entry states. The status answers
   one question only — has this unit been claimed? — and nothing else follows from it.
   <!-- DHF-REQ: openbrain/requirement-867 -->

   | Observed `status` | Meaning | Action |
   |---|---|---|
   | `approved` | not yet claimed; no unit work exists | proceed; step 2 writes `in_progress`. |
   | `in_progress` | already claimed | proceed; step 2 skips the status write. |
   | anything else | not eligible | **halt** and report the actual status. |

   **The status says the unit is claimed. The branch says how much work exists.** Never
   substitute one for the other. `in_progress` spans the whole range — from zero work
   milliseconds after the claim write, through every rework round after N reviewer
   hand-backs, to complete-and-ready-for-review — so it is evidence of nothing beyond the
   claim. A unit at `in_progress` with an empty branch is not a defect and not a
   contradiction: it is simply early, which is the state every `dev` run occupies
   immediately after its own claim write.

   Do not compute a resumability verdict here. **Step 2's resume check reads the branch**
   and is the only thing that answers what is already done; it behaves identically at zero
   commits, at one, and after three hand-backs, so there is nothing for this step to add
   and no case for it to get wrong.

4. Read this round's recorded findings, if there are any. A `formal_review` with
   `outcome: follow_up_required` naming this change request in `subject_refs` is a round
   that a reviewer, a post-merge revert (either the `merge` verb's own in-session gate or
   the runner-owned gate that runs after `merge` has exited), or a `verify` no-op reopen
   routed back to `dev`. **Which actor wrote it does not change what you do**: the findings
   in the review are this round's work either way. Read it with
   `list_formal_review product=keel` (or `search_formal_review`) and match
   `subject_refs` on `keel/change_request-<seq>`; the newest matching row is the
   one that routed this unit back.

   A blocking review carries **findings**, never a claim about progress. Its **absence is
   not a halt condition** — it means there are no recorded findings, which is the normal
   case for a first round. It is likewise legitimate for the corrective work of a round to
   have been applied outside a `dev` child, by the run-queue supervisor or by hand; step 2's
   resume check recognizes it.
5. Resolve the requirement refs kind-aware (see `../SKILL.md` § acceptance contract) —
...
2. Claim the unit. **When step 1 observed `in_progress` the unit is already claimed: skip
   this write entirely and continue at sub-step 3**; re-writing the same status is a no-op
   that only risks a spurious rejection. On a fresh claim,
...
3. **Resume check — the branch is the authority on what is already done.**
